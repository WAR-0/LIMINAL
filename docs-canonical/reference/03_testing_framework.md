...
### Harness Assumptions and SLA Triggers

- Priority system: 4+1 levels (`Info`, `Coordinate`, `Blocking`, `Critical`, `DirectorOverride`).
- Clone [Context Snapshot](./00_glossary.md#context-snapshot): <10ms average, <15ms p99; diff threshold 100 logical changes.
- Territory lookups: O(1), <0.1ms p99 using 2D `TerritoryHash`.
- Escalation defaults under test
  - Deadlock timeout: 60s; lease queue_depth threshold: 2.
  - Consensus timeout: 300s; Blocking wait > 60s escalates.
  - Priority inflation: >50% high‑tier usage in 60s → downgrade + alert.
  - Sustained health breach 60s: message p99 > 1ms; clone spawn p99 > 15ms; critical queue > 10.
  - Build/test failure signals → immediate escalation to [Director Agent](./00_glossary.md#director-agent).
...
### 2.2 [Clone Discussion](./00_glossary.md#clone-based-discussion) Lifecycle
...
- ✅ [Territory Leasing](./00_glossary.md#territory-leasing) lookup: O(1) complexity, <0.1ms
...
### Functional Coverage
- ✅ All [Priority Levels](./00_glossary.md#priority-levels) tested
