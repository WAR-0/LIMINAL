...
This document serves as the authoritative technical specification for LIMINAL's revolutionary architecture, centered around the [**Unified Message Router**](./00_glossary.md#unified-message-router) that enables human-like collaboration between AI agents.
...
| [Context Snapshot](../reference/00_glossary.md#context-snapshot) | 500/sec | <10ms | <10KB | <5% |
...
*   **[Clone-Based Discussions](./00_glossary.md#clone-based-discussion):** Parallel conversation threads that don't interrupt primary work.
...
For a detailed visual guide to the states a lease can be in, see the [Lease Lifecycle Diagram](./03_lease_lifecycle.md).
...
All system parameters are defined in the canonical [**`liminal.config.yaml`**](../config/liminal.config.yaml) file. Below is an illustrative snippet.
...
liminal_config:
  ...
  performance:
    routing_latency_target: 1ms
    clone_spawn_target: 10ms
...
nt territory management.*
t clone spawning, and intelligent territory management.*
