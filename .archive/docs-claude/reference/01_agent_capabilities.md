...
function evaluateLeaseTransfer(factors: LeaseDecisionFactors): LeaseDecision {
    ...
    // Near completion - defer (configurable threshold)
    const deferralThreshold = config.lease_deferral_threshold || 30; // seconds
    if (factors.timeRemaining < deferralThreshold || factors.currentTaskProgress > 80) {
        return LeaseDecision.Defer(factors.timeRemaining);
    }

    // Multiple waiters - escalate for fairness
    const escalationThreshold = config.lease_conflict_escalation_threshold || 2;
    if (factors.queuedRequests > escalationThreshold) {
        return LeaseDecision.Escalate;
    }
...
class ContextManager {
    ...
    // Differential snapshots for efficiency (<10ms target)
    prepareDifferentialSnapshot(lastSnapshot: ContextSnapshot): DiffSnapshot {
        ...
        const threshold = config.diff_snapshot_change_threshold || 100;

        if (changes.size < threshold) {  // Small diff
            return {
                type: 'differential',
...
enum MessagePriority {
    ...
    DirectorOverride // Human Director intervention only
}
...
class AdaptivePriority {
    ...
    adjustPriorityOverTime(message: QueuedMessage): MessagePriority {
        ...
        const escalationTimeout = config.human_escalation_timeout || 300000; // 5 minutes
        if (waitTime > escalationTimeout) {
...
flowchart TD
    ...
    ForceRequest --> HumanReview[Escalate to Human Director]
...
flowchart TD
    ...
    CheckTimeout -->|Yes| EscalateHuman[Escalate to Human Director]
...
4. **Snapshot Efficiently**: Use differential snapshots when possible (<100 changes)
...
8. **Respect Thresholds**: Honor configurable escalation and timeout values
...
interface AgentMetrics {
    ...
    // Performance targets
    cloneSpawnTime: number;          // Target: <10ms
    messageRoutingLatency: number;   // Target: <1ms
}
...
class MetricsCollector {
    collect(): AgentMetrics {
        return {
            ...
            cloneSpawnTime: this.measureCloneSpawnTime(),
            // ... etc
        };
    }
    ...
    reportAnomalies(metrics: AgentMetrics): void {
        ...
        if (metrics.cloneSpawnTime > 10) {
            this.error('Clone spawn time exceeds 10ms target');
        }
    }
}
```

## 7. Director Agent Special Capabilities

The AI Director Agent has additional capabilities and responsibilities:

```typescript
class DirectorAgentCapabilities extends LiminalAgent {
    // Director can use higher priorities
    async sendDirective(directive: Directive): Promise<void> {
        await this.router.send({
            ...directive,
            priority: MessagePriority.Blocking,  // Can use up to Blocking
            sender: 'director_agent'
        });
    }

    // Director handles escalations
    async handleEscalation(escalation: Escalation): Promise<Resolution> {
        // Attempt autonomous resolution first
        if (this.canResolveAutonomously(escalation)) {
            return this.resolveAutonomously(escalation);
        }

        // Escalate to Human Director if needed
        if (this.requiresHumanIntervention(escalation)) {
            return this.escalateToHuman(escalation);
        }

        // Otherwise, orchestrate agent consensus
        return this.orchestrateConsensus(escalation);
    }

    // Director monitors system health
    async monitorSystemHealth(): Promise<void> {
        const metrics = await this.collectSystemMetrics();

        if (metrics.queuedMessages > 1000) {
            this.raiseAlert('Message queue backlog detected');
        }

        if (metrics.deadlockedAgents > 0) {
            this.initiateDeadlockResolution();
        }
    }
}
```
...
- Meet critical performance targets (<10ms clone spawn, <1ms routing)

The key principle is that agents should behave like considerate team members: respecting others' work, communicating asynchronously when possible, and escalating to the AI Director or Human Director only when necessary.